...
class BiblePromisesDialogScreen
{
  ...
  void showDialogMethod({BuildContext context})
  {
    showDialog(
        ...
        builder: (BuildContext context) {
          return Dialog(
            insetPadding: EdgeInsets.only(left: 18.0,right: 18.0,top: MediaQuery.of(context).size.height*0.1,bottom: MediaQuery.of(context).size.height*0.1),
  ...
  Widget _bibleCategoryWidget(BuildContext context,int index)
  {
    ...
    return index.isEven || index == AppStrings.BIBLECATEGORIES.length - 1 ?
      Padding(
      padding: EdgeInsets.only(top:index == 0 ? 25.0 : 8.0,bottom: index == AppStrings.BIBLECATEGORIES.length - 1 ? 25.0 : 8.0),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
            Spacer(),
  ...
  Widget _bibleButtonWidget(BuildContext context,int indexNo)
  {
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: EdgeInsets.only(top: 14.0,bottom: 14.0,left: 2.0,right: 2.0),
        ...
        child: Center(child: Text(AppStrings.BIBLECATEGORIES[indexNo],style: TextStyle(color: AppColors.WHITE_COLOR,fontWeight: FontWeight.w700),textScaleFactor: 0.97,)),
      ),
    );
  }
}
